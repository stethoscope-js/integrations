...
export const summary = async () => {
  if (
    (await pathExists(join(".", "data", "twitter-tweets", "daily"))) &&
    (await lstat(join(".", "data", "twitter-tweets", "daily"))).isDirectory()
  ) {
    const years = (await readdir(join(".", "data", "twitter-tweets", "daily"))).filter((i) => /^\d+$/.test(i));
    ...
    for await (const year of years) {
      ...
      const months = (await readdir(join(".", "data", "twitter-tweets", "daily", year))).filter((i) => /^\d+$/.test(i));
      for await (const month of months) {
        ...
        const days = (await readdir(join(".", "data", "twitter-tweets", "daily", year, month))).filter((i) =>
          /^\d+$/.test(i)
        );
        for await (const day of days) {
          let json = await readJson(join(".", "data", "twitter-tweets", "daily", year, month, day, "tweets.json"));
        ...
        if (Object.keys(dailyData).length)
          await write(
            join(".", "data", "twitter-tweets", "summary", "days", year, `${month}.json`),
      ...
      if (Object.keys(monthlyData).length)
        await write(
          join(".", "data", "twitter-tweets", "summary", "months", `${year}.json`),
          JSON.stringify(monthlyData, null, 2)
        );
      if (yearlySum) yearData[parseInt(year)] = yearlySum;
    }
    if (Object.keys(yearData).length)
      await write(join(".", "data", "twitter-tweets", "summary", "years.json"), JSON.stringify(yearData, null, 2));
    for await (const year of Object.keys(weeklyData)) {
      for await (const week of Object.keys(weeklyData[year])) {
        if (
          ...
          await write(
            join(".", "data", "twitter-tweets", "summary", "weeks", year, `${week}.json`),
